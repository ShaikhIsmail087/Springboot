...
public class UserService {
    ...
    private User createUserFromUserEntity(UserEntity ue) {
        ...
        // Flatten the stream  of stream to get the set of authoritis

        Set<String> authorities = streamStreamAuths.flatMap(authStream -> authStream)
                .collect(Collectors.toSet());

        // Now add the Role (from the User_Role table) as authorities bcuz UserDetails does not support adding
        // Role separately as it does not have any setRole(String... roles)
